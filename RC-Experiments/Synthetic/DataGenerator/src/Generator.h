...
void Generator::parseT1Attributes(){
    ...
    for (int i = 0; i < nrT1Atts; ++i)
    {
        ...
        attDistance = ATTCOMPARATOR(dictionary[sfp.get_token(6)]); // EUCLIDIAN
...
void Generator::distributeGroups(){
    ...
    for(int i = nrGroups; i < nrT1; i++){
        int idGroup = getRandomGroup(startGroupsDist);
        if(idGroup > 0 and idGroup < nrT1)
            tGroups[idGroup].insert(i);
    }
